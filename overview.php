 ...
 *
 * Landing page that explains the two mechanisms — send (push) and allow
 * retrieval (pull) — side-by-side, with a live "configured / not configured"
 * badge on each so an operator can see at a glance what's set up.
 *
...
echo $OUTPUT->box(get_string('overview_intro', 'local_sentinel'));
...
echo html_writer::tag(
    'p',
    html_writer::tag('strong', 'When to use: ') . s(get_string('overview_send_when', 'local_sentinel')),
    ['class' => 'mb-2']
);
echo html_writer::tag(
    'p',
    html_writer::tag('strong', 'Requires: ') . s(get_string('overview_send_requires', 'local_sentinel')),
    ['class' => 'mb-3']
);
echo html_writer::link(
    $sendurl,
    s(get_string('overview_send_cta', 'local_sentinel')),
...
echo html_writer::tag(
    'p',
    html_writer::tag('strong', 'When to use: ') . s(get_string('overview_pull_when', 'local_sentinel')),
    ['class' => 'mb-2']
);
echo html_writer::tag(
    'p',
    html_writer::tag('strong', 'Requires: ') . s(get_string('overview_pull_requires', 'local_sentinel')),
    ['class' => 'mb-3']
);
echo html_writer::link(
    $pullurl,
    s(get_string('overview_pull_cta', 'local_sentinel')),
    ['class' => 'btn btn-primary mt-auto align-self-start']
);
echo html_writer::end_div();
echo html_writer::end_div();
...
echo html_writer::tag(
    'h4',
    s(get_string('overview_notes_heading', 'local_sentinel')),
    ['class' => 'h6 text-uppercase text-muted mt-4']
);
echo html_writer::start_tag('ul');
echo html_writer::tag('li', s(get_string('overview_note_both', 'local_sentinel')));
echo html_writer::tag('li', s(get_string('overview_note_pull', 'local_sentinel')));
echo html_writer::tag('li', s(get_string('overview_note_push', 'local_sentinel')));
